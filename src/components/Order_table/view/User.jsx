...
export default function UserView() {
  ...
  useEffect(()=>{
    ...
    const fetchdata=()=>{
      ...
      .then(re=>{
     if(!ignore)setOrdersValue(re.data)})
    }
    fetchdata()
    return()=>{
      ignore=true
...
const handleClose = () =>{
  ...
  const Update=()=>{
    const design=data.design?data.design:datamodif.design
    const delivery=data.delivery_address?data.delivery_address:datamodif.delivery_address
    axios.put(API_URL+"/orders/"+idorder,{
      data:{
        "design":design,
  ...
  const Submit=(e)=>{
    e.preventDefault()
    datamodif.design?Update():Addorder()
    setOpen(false)
  }
